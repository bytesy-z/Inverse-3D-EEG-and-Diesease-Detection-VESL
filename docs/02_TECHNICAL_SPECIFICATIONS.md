...
| `FS_SIMULATION` | 1000 Hz | Internal TVB simulation step, downsampled to 200 Hz for EEG output. |
...
```python
# Pseudocode — exact implementation in tvb_simulator module
simulator = tvb.simulator.Simulator(
    ...
    integrator=HeunStochastic(dt=1.0, noise=Additive(nsig=D)),
    monitors=[TemporalAverage(period=5.0)],  # 200 Hz output
    simulation_length=10000.0               # 10 seconds per sample
)
```

**Integration method**: Heun stochastic (second-order Runge-Kutta with additive noise). This is the standard integrator for the Epileptor in TVB (Sanz-Leon et al., 2013).
...
1. **Number of epileptogenic regions**: $k \sim \text{DiscreteUniform}(1, 8)$
2. **Which regions are epileptogenic**: $k$ regions selected uniformly at random, with optional spatial clustering bias (50% chance of selecting a spatially contiguous cluster using the connectivity adjacency).
3. **Excitability assignment**:
   - Epileptogenic regions: $x_{0,i} \sim \text{Uniform}(-1.8, -1.2)$
   - Healthy regions: $x_{0,i} \sim \text{Uniform}(-2.2, -2.05)$
...
1. Configure TVB simulator with sampled parameters.
2. Run simulation for 12 seconds (first 2 seconds discarded as transient).
